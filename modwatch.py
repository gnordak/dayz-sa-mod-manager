...
silent = False
reboot_enabled = True
steam_api_id = 'xxxxxxxxxxxxxxxxxxxx'

mods = []

with open("C:\DayZServer\Modlist.txt", "r") as modlist_file:
    for line in modlist_file:
        mods.append(line.strip())

#workshop_prefix = 'https://steamcommunity.com/sharedfiles/filedetails/?id='
...
if silent is False:
    #send("Server is booting up. This may take 2-3 minutes depending on mod updates.")

    print("Sleeping 3 min for server to start.")
    time.sleep(180)
...
while True:
    for mod in mods:
        ...
        #time.sleep(5)
        mod_id = mod.split(',')[0]
        mod_name = mod.split(',')[1].strip('@')
        #url = workshop_prefix + mod_id
        #page = requests.get(url)
        #soup = BeautifulSoup(page.content, 'html.parser')
        #results = soup.find_all(class_='detailsStatRight')
        #date_updated = results.pop().text
        #print('\n')
        #print("Querying steam API for", mod_name, mod_id)
        wfile = steam.get_workshop_file(mod_id)
        #print("Workshop Updated:", wfile.updated)
        #print('\n')
        
        try:
            date = dt.datetime.strptime(wfile.updated, '%b %d @ %I:%M%p')
            date = date.replace(year=2020)
        except:
            date = dt.datetime.strptime(wfile.updated, '%b %d, %Y @ %I:%M%p')
            
        print(date)
        now = dt.datetime.now()
        #try:
        #    #no year
        #    date = dt.datetime.strptime(date_updated, '%b %d @ %I:%M%p')
        #    date = date.replace(year=2020)

        #except:
        #    date = dt.datetime.strptime(date_updated, '%b %d, %Y @ %I:%M%p')
            
        sec_since = (now - date).total_seconds()

        if sec_since < 360:
            print('\n')
            print(sec_since, "seconds since update")
            print("[OUT OF DATE]", mod_name, (now - date))           
            if silent is False:
                send(f"Mod {mod_name} was just updated so the server is rebooting. Please restart your launchers and verify your mods if needed.")
            if reboot_enabled is True:
                os.system("shutdown -t 0 -r -f")
        else:
            print('\n')
            print("[GOOD]", mod_name, "\t", (now - date), (now - date).total_seconds())
